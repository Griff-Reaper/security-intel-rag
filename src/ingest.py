...
# Resolve sibling modules and data files relative to this file rather than the
# working directory, so ingestion behaves the same from any directory.
...
class SecurityDataIngester:
    ...
    def __init__(self, persist_directory: str = DEFAULT_PERSIST_DIRECTORY, collection_name: str = "security_intel"):
        """
        ...
        # PersistentClient writes to disk. chromadb.Client() is ephemeral (in-memory)
        # and ignores persist_directory, so anything ingested through it is lost when
        # the process exits and query.py can never find the collection.
    ...
    @staticmethod
    def format_cve_document(cve: Dict[str, Any]) -> str:
        """
...
"""
    ...
    @staticmethod
    def format_threat_intel_document(threat: Dict[str, Any]) -> str:
        """
...
"""
    ...
    def ingest_cves(self, json_file: str) -> int:
        ...
        # Add to ChromaDB. upsert (not add) so re-running ingestion overwrites
        # existing records instead of failing on duplicate IDs.
        ...
        print(f"[OK] Successfully ingested {len(cves)} CVEs")
        return len(cves)
    ...
    def ingest_threat_intel(self, json_file: str) -> int:
        ...
        print(f"[OK] Successfully ingested {len(threats)} threat reports")
        return len(threats)
...
if __name__ == "__main__":
    ...
    # Data files are resolved from the project root, so ingestion works
    # regardless of the directory the script is invoked from.
    cve_file = str(PROJECT_ROOT / "data" / "sample_cves.json")
    threat_file = str(PROJECT_ROOT / "data" / "threat_intel.json")

    for path in (cve_file, threat_file):
    ...
    print("\n[OK] Database is ready for queries!")
